...
import com.example.coffwaiter.ui.status.StatusPage
...
class PrefUtil {
    companion object {
        ...
        fun getTimerState(context: Context): StatusPage.TimerState{
            val preferences = PreferenceManager.getDefaultSharedPreferences(context)
            val ordinal = preferences.getInt(TIMER_STATE_ID, 0)
            return StatusPage.TimerState.values()[ordinal]
        }

        fun setTimerState(state: StatusPage.TimerState, context: Context){
            val editor = PreferenceManager.getDefaultSharedPreferences(context).edit()
